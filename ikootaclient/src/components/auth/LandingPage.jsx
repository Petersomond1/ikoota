...
//   const [activeFeature, setActiveFeature] = useState(0);
//   const navigate = useNavigate();
//   const { isAuthenticated, getDefaultRoute, loading } = useUser();

//   // ✅ FIXED: Allow pending users to stay on landing page, only redirect active members
//   useEffect(() => {
//     // Don't redirect while still loading user data
//     if (loading) return;
...
//               </ul>
//               <button 
//                 onClick={isAuthenticated ? () => navigate(getDefaultRoute()) : handleJoinCommunity} 
//                 className="btn-tier primary"
//               >
//                 {isAuthenticated ? 'Access Your Account' : 'Apply for Membership'}
//               </button>
//             </div>
...
//   // ✅ FIXED: Only redirect authenticated users after loading is complete
//   // and only if they're not already on the landing page
//   useEffect(() => {
//     // Don't redirect while still loading user data
//     if (loading) return;
    
//     // Only redirect if user is authenticated and has completed loading
//     if (isAuthenticated && !loading) {
//       const defaultRoute = getDefaultRoute();
//       // Only redirect if the default route is different from current page
//       if (defaultRoute && defaultRoute !== '/') {
//         console.log('🔄 Authenticated user on landing page, redirecting to:', defaultRoute);
//         navigate(defaultRoute);
//       }
//     }
