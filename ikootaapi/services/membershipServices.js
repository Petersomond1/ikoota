...
// ===============================================
// MEMBERSHIP SERVICES - UPDATED FOR MODULAR ARCHITECTURE
// Business logic layer for membership operations
// ===============================================
 ...
 * Used by: adminManagementController.getPendingApplications
 */
 ...
 * Used by: adminManagementController.getAllReports
 */
  ...
  try {
    ...
    const [conversionFunnel] = await db.query(`
      ...
      SELECT 
        ...
        ROUND(COUNT(DISTINCT sl.user_id) * 100.0 / (SELECT COUNT(*) FROM users WHERE role = 'user'), 2) as percentage
      FROM surveylog sl
  ...
  try {
    const [journey] = await db.query(`
      SELECT 
        ...
        full_app.approval_status as full_status,
      ...
      LEFT JOIN surveylog full_app ON u.id = CAST(full_app.user_id AS UNSIGNED)
        ...
        AND full_app.id = (
          SELECT MAX(id) FROM surveylog 
          WHERE user_id = u.id AND application_type = 'full_membership'
        )
 ...
 * Used by: preMemberApplicationController, adminManagementController
 */
  ...
  try {
    ...
    const [metrics] = await db.query(`
      SELECT 
        ...
        COUNT(DISTINCT CASE WHEN full_sl.approval_status = 'approved' THEN u.id END) as approved_full_members,
        ROUND(COUNT(DISTINCT CASE WHEN full_sl.approval_status = 'approved' THEN u.id END) * 100.0 / COUNT(DISTINCT CASE WHEN full_sl.id IS NOT NULL THEN u.id END), 2) as full_approval_rate,
        
        -- Overall Conversion
        ROUND(COUNT(DISTINCT CASE WHEN full_sl.approval_status = 'approved' THEN u.id END) * 100.0 / COUNT(DISTINCT u.id), 2) as overall_conversion_rate
        
      FROM users u
      LEFT JOIN surveylog sl ON u.id = CAST(sl.user_id AS UNSIGNED) 
        AND sl.application_type = 'initial_application'
      LEFT JOIN surveylog full_sl ON u.id = CAST(full_sl.user_id AS UNSIGNED) 
        AND full_sl.application_type = 'full_membership'
 ...
 * Used by: adminManagementController
 */
 ...
 * Used by: adminManagementController notification functions
 */
...
/**
 * Get membership pipeline overview
 * Used by: adminManagementController.getMembershipOverview
 */
export const getMembershipPipelineOverview = async () => {
  try {
    const [pipelineData] = await db.query(`
      SELECT 
        'New Registrations' as stage_name,
        'none' as membership_stage,
        COUNT(*) as count,
        DATEDIFF(NOW(), MIN(u.createdAt)) as oldest_days,
        COUNT(CASE WHEN u.createdAt >= DATE_SUB(NOW(), INTERVAL 7 DAY) THEN 1 END) as recent_count
      FROM users u
      WHERE u.membership_stage = 'none' OR u.membership_stage IS NULL
      
      UNION ALL
      
      SELECT 
        'Pending Applications' as stage_name,
        'applicant' as membership_stage,
        COUNT(*) as count,
        DATEDIFF(NOW(), MIN(sl.createdAt)) as oldest_days,
        COUNT(CASE WHEN sl.createdAt >= DATE_SUB(NOW(), INTERVAL 7 DAY) THEN 1 END) as recent_count
      FROM surveylog sl
      WHERE sl.approval_status = 'pending' AND sl.application_type = 'initial_application'
      
      UNION ALL
      
      SELECT 
        'Pre-Members' as stage_name,
        'pre_member' as membership_stage,
        COUNT(*) as count,
        DATEDIFF(NOW(), MIN(u.createdAt)) as oldest_days,
        COUNT(CASE WHEN u.createdAt >= DATE_SUB(NOW(), INTERVAL 7 DAY) THEN 1 END) as recent_count
      FROM users u
      WHERE u.membership_stage = 'pre_member'
      
      UNION ALL
      
      SELECT 
        'Full Members' as stage_name,
        'member' as membership_stage,
        COUNT(*) as count,
        DATEDIFF(NOW(), MIN(u.createdAt)) as oldest_days,
        COUNT(CASE WHEN u.createdAt >= DATE_SUB(NOW(), INTERVAL 7 DAY) THEN 1 END) as recent_count
      FROM users u
    ...
    throw new CustomError(`Failed to get pipeline overview: ${error.message}`, 500);
  }
};
  ...
  try {
    const [activityData] = await db.query(`
      SELECT 
        ...
        COUNT(DISTINCT CASE WHEN sl.application_type = 'initial_application' THEN sl.id END) as initial_applications,
        COUNT(DISTINCT CASE WHEN sl.application_type = 'full_membership' THEN sl.id END) as full_applications,
        
        -- Status progression
        MAX(CASE WHEN sl.application_type = 'initial_application' THEN sl.createdAt END) as last_initial_app,
        MAX(CASE WHEN sl.application_type = 'full_membership' THEN sl.createdAt END) as last_full_app,
        ...
        DATEDIFF(NOW(), MAX(CASE WHEN sl.application_type = 'initial_application' AND sl.approval_status = 'approved' THEN sl.reviewedAt END)) as days_since_approval
        
      FROM users u
      LEFT JOIN surveylog sl ON u.id = CAST(sl.user_id AS UNSIGNED)
      LEFT JOIN full_membership_access fma ON u.id = fma.user_id
      WHERE u.id = ?
      GROUP BY u.id, u.username, u.membership_stage, u.is_member, u.createdAt, 
               fma.first_accessedAt, fma.last_accessedAt, fma.access_count
    `, [userId]);
    ...
    return {
      user_activity: activityData[0],
      generatedAt: new Date().toISOString()
    };

  } catch (error) {
    console.error('❌ Error getting user activity summary:', error);
    throw new CustomError(`Failed to get user activity: ${error.message}`, 500);
  }
};

/**
 * Get system performance metrics
 * Used by: adminManagementController.getSystemConfig
 */
  ...
  try {
    const [performanceData] = await db.query(`
      SELECT 
        ...
        COUNT(CASE WHEN u.createdAt >= DATE_SUB(NOW(), INTERVAL 24 HOUR) THEN 1 END) as new_users_today,
        ...
        COUNT(CASE WHEN u.membership_stage = 'member' THEN 1 END) as active_members,
    ...
    throw new CustomError(`Failed to get performance metrics: ${error.message}`, 500);
  }
};
