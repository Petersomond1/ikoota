...
export const createCommentService = async ({ user_id, chat_id, teaching_id, comment, media = [], parent_comment_id = null }) => {
  ...
  try {
    ...
    if (parent_comment_id) {
      const parentComment = await connection.query('SELECT chat_id, teaching_id FROM comments WHERE id = ?', [parent_comment_id]);
      if (!parentComment[0]) {
    ...
    const [result] = await connection.query(
      `INSERT INTO comments (
        user_id, chat_id, teaching_id, comment, parent_comment_id,
...
export const deleteCommentById = async (commentId) => {
  try {
    ...
    await db.query('DELETE FROM comments WHERE parent_comment_id = ?', [commentId]);
  ...
  try {
    ...
    const query = `
      WITH RECURSIVE comment_tree AS (
        ...
        SELECT c.id, c.parent_comment_id, ct.depth + 1
        FROM comments c
